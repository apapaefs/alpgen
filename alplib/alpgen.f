...
c     
 ...
 2    read(5,'(A)',end=3,err=3) parline
c     Handle LHAPDF set name: 'lhapdfst CT14nlo'
      read(parline,*,err=10) chparam
      if(chparam(1:8).eq.'lhapdfst') then
         read(parline,*,err=2) chparam,lhapdfset
         write(6,*) 'LHAPDF set name: ',trim(lhapdfset)
         if(imode.eq.1) write(niopar,'(A,1X,A)')
     $        'lhapdfst',trim(lhapdfset)
         goto 2
      endif
c     Standard parameter: read name and numeric value
      read(parline,*,err=10) chparam,chvalue
...
C first read in parameters from the imode=1 run
      itmp=0
 1    read(niopar,'(A)',end=2,err=2) parline
      read(parline,*,err=11) chparam,chvalue
      goto 12
c     Handle lines where second field is not a number (e.g. lhapdfst)
 11   read(parline,*,err=1) chparam
 12   continue
 10   if(chparam(1:3).eq.'eoi') goto 2
      if(chparam(1:3).eq.'***') goto 1
c     Handle LHAPDF set name string parameter
      if(chparam(1:8).eq.'lhapdfst') then
         read(parline,*) chparam,lhapdfset
         write(6,*) 'Read LHAPDF set name: ',trim(lhapdfset)
         goto 1
      endif
      if(itmp.eq.0) then
        itmp=1
...
c hidden parameters
     ...
     $   ='impunw=2: maxwgt<0, impunw=1: ask user, impunw=0: default'
      ...
      enddo
c
c LHAPDF interface parameters
      chpar(196)='ilhapdf'
      chpdes(196)='LHAPDF mode: 0=built-in PDFs, 1=use LHAPDF6'
      partyp(196)=1
      parval(196)=0
      do i=1,nactprc
        paruse(196,i)=1
...
c
      chpar(197)='lhapdfid'
      chpdes(197)='LHAPDF member number (0=central value)'
      partyp(197)=1
      parval(197)=0
      do i=1,nactprc
        paruse(197,i)=1
...
c initialise QCD and PDF parameters
      if(ilhapdf.eq.1) then
c     LHAPDF6 mode: initialise the PDF set via LHAPDF
         write(6,*) 'Using LHAPDF6 with set: ',
     $        trim(lhapdfset),' member:',lhapdfmem
         call lhapdf_init(lhapdfset,lhapdfmem)
c     Get alpha_s(MZ) and PDF metadata from LHAPDF
         call lhapdf_getlam(xlam,nloop,sche)
         nmnr=-1
         pdftyp='LHAPDF:'//lhapdfset(1:17)
