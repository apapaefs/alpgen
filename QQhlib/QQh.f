...
c       jgrid= 5   -->   q q -> q q
      data jgrid/1,2,3,4,2*1,2*5,4*2,3,4,3,4,3,4,82*0/
      character*2 Q(4:6)
...
c     process input parameters
      ...
      elseif(njets.eq.2) then
         ...
         ngrid=5
         navg=1
      elseif(njets.ge.3.and.njets.le.4) then
         jprocmax=18
         ngrid=5
         navg=1
      else
         write(*,*) 'njets=',njets,' not available'
         stop
      endif
      if(ihvy.eq.5) then
        idecay='n'
...
c
     ...
     $       ,' Higgs'
      ...
      write(niosta,*)
     $     '======================================='
      write(niosta,*) 'Heavy quark mass:'
      write(niosta,*) 'm(',Q(ihvy),')=',mq
      write(niosta,*) 'Higgs mass:'
      write(niosta,*) 'm(H)=',mH
      write(niosta,*)
...
c     jproc
c
c---Q Qbar H (+gluons) final state
c
c  1  g g     -> Q Qbar h            (+ gluons)
c  2  q qbar  -> Q Qbar h            (+ gluons)   q=u,d,c,s
c     qbar q  -> Q Qbar h            (+ gluons)   q=u,d,c,s
c  3  g q     -> Q Qbar h q          (+ gluons)   and barred
c  4  q g     -> Q Qbar h qbar       (+ gluons)   and barred
c  5  g g     -> Q Qbar h q qbar     (+gluons)
c  6  g g     -> Q Qbar h b bbar
c  7  q q     -> Q Qbar h q q        (+ gluons)   and barred
c  8  q q'    -> Q Qbar h q q'       (+ gluons)   and barred
c  9  q qbar  -> Q Qbar h q qbar     (+ gluons)
c 10  q qbar' -> Q Qbar h q qbar'    (+ gluons)
c 11  q qbar  -> Q Qbar h q' qbar'   (+ gluons)
c 12  q qbar  -> Q Qbar h b bbar     (+ gluons)
c 13  g q     -> Q Qbar h q qbar q   (+ gluons)
c 14  q g     -> Q Qbar h q qbar q   (+ gluons)
c 15  g q     -> Q Qbar h q' qbar' q (+ gluons)
c 16  q g     -> Q Qbar h q' qbar' q (+ gluons)
c 17  g q     -> Q Qbar h b bbar q   (+ gluons)
c 18  q g     -> Q Qbar h b bbar q   (+ gluons)
c
c-----------------------------------------------------------------
      implicit none
      include 'alpgen.inc'
      ...
      integer i,j,k,icount,init,jproc,ng
      real cfac(0:6),ifact(0:6)
      data cfac/8e0,6*3e0/,init/0/
      integer imap(-4:4)
      data imap/-2,-1,-2,-1,0,1,2,1,2/
      integer itmp,it,itb,ib,ibb,iflaux,iltaux,ilbaux
c
c   for top decay
c
...
c
c     jproc=2   njets>=0
c  2  q qbar -> Q Qbar h + njets g
c     and qbar q -> Q Qbar h + njets g   q=u,d,c,s
...
c
c     jproc=3   njets>=1
c  3  g q -> Q Qbar h q + (njets-1) g
c     and g qbar -> Q Qbar h qbar + (njets-1) g   q=u,d,c,s
...
c
c     jproc=4   njets>=1
c  4  q g -> Q Qbar h q + (njets-1) g
c     and qbar g -> Q Qbar h qbar + (njets-1) g   q=u,d,c,s
...
c
c     jproc=5   njets>=2
c 5   g g -> Q Qbar h q qbar + (njets-2) g  (q.ne.b)
      else if(jproc.eq.5) then
         slum=f1(0)*f2(0)
c        to account for 4 possible f.s.
         slum=slum*4.d0
         icount=0
         do i=-4,4
            if(i.ne.0) then
...
c
c     jproc=6   njets>=2
c 6   g g -> Q Qbar h b bbar + (njets-2) g
         ...
         itmp=1
         effco = ccoef(2,njets)
c        to account for possible 2 b's and 2 bbar's
         if(ihvy.eq.5) slum= slum/4.d0
         goto 100
c
c     jproc=7   njets>=2
c  7  q q -> Q Qbar h q q + (njets-2) g
c     and qbar qbar -> Q Qbar h qbar qbar + (njets-1) g   q=u,d,c,s
...
c                 to account for 2 id. quarks in the f.s.
                  slum= slum/2.d0
                  goto 100
               endif 
            endif
...
c
c     jproc=8   njets>=2
c  8  q q' -> Q Qbar h q q' + (njets-2) g
...
c
c     jproc=9   njets>=2
c  9  q qbar -> Q Qbar h q qbar + (njets-2) g
...
c
c     jproc=10   njets>=2
c 10  q qbar' -> Q Qbar h q qbar' + (njets-2) g
         ...
         enddo
c     jproc=11   njets>=2
c 11  q qbar -> Q Qbar h q' qbar' + (njets-2) g
...
c
c     jproc=12   njets>=2
c 12  q qbar -> Q Qbar h b bbar + (njets-2) g
...
c
c     jproc=13   njets>=3
c 13  g q -> Q Qbar h q q qbar+ (njets-3) g
c     and g qbar -> Q Qbar h qbar q qbar + (njets-3) g   q=u,d,c,s
         ...
         do i=-4,4
            if(i.ne.0) then
               ...
               if(tmptot.ge.rn) then 
                  ...
                  effco = ccoef(3,njets-2)
c
c to account for the presence of two indistinguishable quarks
c
                  slum= slum/2.d0
                  goto 100
               endif 
...
c
c     jproc=14   njets>=3
c 14  q g -> Q Qbar h q q qbar+ (njets-3) g
c     and qbar g -> Q Qbar h qbar q qbar + (njets-3) g   q=u,d,c,s
...
c
c     jproc=15   njets>=3
c 15  g q -> Q Qbar h q q' qbar'+ (njets-3) g
c     and g qbar -> Q Qbar h qbar q qbar + (njets-3) g   q=u,d,c,s
...
c
c     jproc=16   njets>=3
c 16  q g -> Q Qbar h q q' qbar'+ (njets-3) g
c     and qbar g -> Q Qbar h qbar q qbar + (njets-3) g   q=u,d,c,s
...
c
c     jproc=17   njets>=3
c 17  g q -> Q Qbar h q b bbar+ (njets-3) g
c     and g qbar -> Q Qbar h qbar b bbar + (njets-3) g   q=u,d,c,s
...
c
c     jproc=18   njets>=3
c 18  q g -> Q Qbar h q b bbar+ (njets-3) g
c     and qbar g -> Q Qbar h qbar b bbar + (njets-3) g   q=u,d,c,s
...
c                                                                     c
c  h(1)  h(2) -> higgs(3) Q(4) anti_Q(5) ... h(njets)                 c
c                                                                     c
...
*
      pswgt=0.d0
      if(ihvy.eq.5)
     + call momgen_bbh(njets,mq,mh,roots,x1,x2,pcm,wgt,lnot)
      if(ihvy.eq.6)
     + call momgen_tth(njets,mq,mb,mh,roots,x1,x2,pcm,wgt,lnot)
...
*
      factor= 1d0/(2.d0*pi)**(3*(njets+3)-4)/2.d0/s/x1/x2
*
...
*
      if(ihvy.eq.5) call chkcut_bbh(lnot,pt,p,eta,dr,njets)
      if(ihvy.eq.6) call chkcut_tth(lnot,pt,p,eta,dr,njets)
...
*
 ...
 100  continue
      ...
      subroutine chkcut_bbh(lnot,pt,p,eta,dr,njets)
...
c     require dR(b-jet)<drjmin, dR(jet-jet)<drjmin
 ...
 10   lnot= 1
      ...
      subroutine chkcut_tth(lnot,pt,p,eta,dr,njets)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c                                                                c
c     Applies kinematical cuts to the final state during the phase
c     -space generation                                          c
...
*
      subroutine momgen_tth(njets,qm,bm,hm,roots,x1,x2,p,wgt,lw)
*
c-    Generator of np particles in the LAB frame.
*
      implicit none
      real*8 qm,bm,hm,roots,x1,x2,wgt,wgt1,wgt2,zero,qm2,etacut
      real*8 ptjmin,cutkin,etajmax,drjmin,s,sw
      real*8 ptQmin,etaQmax,drQmin
...
*
        ...
        enddo
*
c-      Parameters for the Higgs mass
*
        pt0_l(nw-1)= zero
        pt1_l(nw-1)= roots/2.d0
        eta0(nw-1) = etacut
        xm(nw-1)   = hm
*
c-      Parameters for the ttbar system:
*
        cntt      = 1.d0
        pt0_l(nw) = zero
        pt1_l(nw) = roots/2.d0
...
*
      do j=1,np-3
        xm(j)=0.d0        
      enddo
      if(jproc.eq.6.or.jproc.eq.12) then
         do j=1,2
            xm(j)=bm
         enddo
      endif
      if(jproc.eq.17.or.jproc.eq.18) then
         do j=2,3
            xm(j)=bm
         enddo
      endif
...
*
      xm(nw)= sqrt(sw)
      xmsum = xmsum0+xm(nw)
      if(jproc.eq.6.or.jproc.eq.12.or.
     +   jproc.eq.17.or.jproc.eq.18) xmsum= xmsum+2.d0*bm
*
...
*
      ...
      tau0   = max(tau0,1.d0/s*(2.d0*qm+hm
     +                  +float(njets)*ptjmin)**2)
      if (tau0.ge.1.d0) goto 100
*
...
*
      do j= 1,nw
        do k= 0,3
          if (j.eq.nw) then
            pm(k,j) = pm(k,j)*roots
          elseif(j.eq.nw-1) then
            p(k,3)  = pm(k,j)*roots
          else
            p(k,j+5)= pm(k,j)*roots
          endif
        enddo
      enddo
*
...
*
      subroutine momgen_bbh(njets,bm,hm,roots,x1,x2,p,wgt,lw)
*
c-    Generator of np particles in the LAB frame.
*
      implicit none
      real*8 roots,x1,x2,wgt,wgt1,wgt2,zero,etacut,bm,hm,eps
      real*8 cutkin
...
c
c global event cuts
      double precision ptjmin,ptjmax,etajmin,etajmax,drjmin
      real*8 ptbmin,etabmax,drbmin
      integer lw,npar,mpar,np,njets,j,k,lw1,jj
      parameter (npar= 20)
      double precision pm(0:4,npar),pt0(npar),pt1(npar),xm(npar),
     .       xmr(npar),eta0(npar),pt0_l(npar),pt1_l(npar),p(0:3,*)
      integer jprocmax,jproc,ngrid,jgrid,maxsubproc
      parameter (maxsubproc= 100)
      common/procct/jprocmax,jproc,ngrid,jgrid(maxsubproc)
      common/loccut/cutkin(10)
      ...
      save
*
      if ((njets.gt.5).or.(njets.lt.0)) then
...
*
     ...
     +        (2.d0*sqrt(ptbmin**2+bm**2)+hm+float(njets)*ptjmin)**2)
*
...
***     >               11,1,1,1,1,  11,1,2,1,2,  11,3,2,3,2,   2,1,1,1,2,  ! guu, gdd, gbb, w+ud
     >               11,1,1,1,1,  11,1,2,1,2,   11,2,2,2,2,  11,2,1,2,1,    ! guu, gdd, gcc, gss
***     >                1,1,1,1,1,   1,1,2,1,2,    1,2,2,2,2,   1,2,1,2,1,    ! zuu, zdd, zcc, zss
***     >                2,1,1,1,2,   3,1,2,1,1,    2,2,1,2,2,   3,2,2,2,1,    ! wud, wdu, wcs, wsc
     >                11,3,1,3,1,   11,3,2,3,2,                             !gtt,gbb 
...
C
         ...
         do j1=1,3
          do j2=1,4
           j3=j3+nsourfermion(j1,j2)
           if(j1.eq.3.and.j2.eq.2) then
            if(nsourfermion(j1,j2).ne.1) then
             write(6,*)'fatal accident in OTTSPIN'
            else
             lbb=j3
            endif
           endif
...
C
         do j1=1,3
          do j2=1,4
           j3=j3+nsourfermionbar(j1,j2)
           if(j1.eq.3.and.j2.eq.2) then
            if(nsourfermionbar(j1,j2).ne.1) then
             write(6,*)'fatal accident in OTTSPIN'
            else
             lb=j3
            endif
...
C
         call randa(rspin)
C
         if(rspin.lt.0.45) then
          js=sqrt(5.d0)/3.d0
          spinsour(lb)=0.49
          spinsour(lbb)=0.51
         elseif(rspin.lt.0.90) then
          js=sqrt(5.d0)/3.d0
          spinsour(lb)=-0.49
          spinsour(lbb)=-0.51
         elseif(rspin.lt.0.95) then
          js=sqrt(5.d0)
          spinsour(lb)=0.49
          spinsour(lbb)=-0.51
         else
