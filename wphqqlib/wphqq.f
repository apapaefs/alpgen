...
c BEGIN_RPITTAU
     ...
     $         ,' dR(ph-j)>',drphjmin,' dR(ph-l)>',drphlmin  
        write(niosta,*) 'dR(ph-ph)>',drphmin
      endif
      end
c END_RPITTAU
...
c     jproc
c
c     A b b_bar pair is always present in the final state!!!
c
...
c     evaluate spin weight factors
      swgt=2e0
c 
      swgt=swgt**(nfspart+nph+2)
c 
c
c 
      xlum=dble(slum*cwgt*swgt*ifact(ng)*ifact(nph))/resc**nfspart
c 
      if(jproc.eq.14.or.jproc.eq.25.or.jproc.eq.20.or.jproc.eq.18.or
     $     .jproc.eq.15.or.jproc.eq.22.or.jproc.eq.23)  
     $      xlum=0.5d0*xlum ! identical quarks in final state
c     effco=ccoef(#quark pairs,#gluons)
      ...
      xlum=xlum*ccoef(nlqp,2+nfspart-2*nlqp)
c
c
      do i= 1,maxpar 
        afl(i)= ifl(i)
        if (afl(i).eq.21) afl(i)= 0 
      enddo
c
c      if(ifl(il1)+ifl(il2).eq.1) then
c        xlum=1.d-20*xlum
c        return
c      endif
c
c      print*,'il,inu           =',il,inu 
c      print*,'afl(il1),afl(il2)=',afl(il1),afl(il2) 
c      print*,'                  '
c
      end                                                
c     end selflav
...
c assign momenta to the usr common block
      ...
      enddo
      do j=1,nph
         ptphot(j)=pt(nfspart+j+2)
         etaphot(j)=eta(nfspart+j+2)
         do i=j+1,nph
            drphotphot(i,j)=dr(nfspart+i+2,nfspart+j+2)
            drphotphot(j,i)=drphotphot(i,j)
         enddo
      enddo
      do j=1,nfspart
        do i=1,nph
           drjphot(j,i)=dr(j+2,i+2+nfspart)
        enddo
      enddo
      drbb=drjj(ib-2,ibb-2)
      do i=1,4
         pbott(i)=p(i,ib)
         pbbar(i)=p(i,ibb)
      enddo
      ...
      enddo
      do i=1,nph
         drbphot(i)=dr(ib,nfspart+2+i)
         drbbphot(i)=dr(ibb,nfspart+2+i)
      enddo
      do i=1,4
         plep(i)=p(i,il)
         pnu(i)=p(i,inu)
...
c     isolate lepton from b's and light jets
      ...
      enddo 
c     isolate photons from the charged lepton
      do i=1,nph
        if(drlphot(i).lt.drphlmin)   goto 10
      enddo 
      ptbjet=0d0
      return
 10   iflag=1
...
c                                                                c
c  h(1)  h(2) -> h h ... Q Q ph  ... ph e nu                     c
c                                                                c
c  where h is any light-flavoured quark or gluon                 c
c  and Q is an heavy jet                                         c  
c                                                                c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      implicit none
      include 'alpgen.inc'
      ...
      real *8 djpd,factor
c 
      integer  nchand,nchan,npk
      real*8   valu,ran0,bet
      parameter (nchand= 8)
      integer lnar(1:nchand)
      real *8 apw(1:nchand),whar(1:nchand),dj
      data apw/nchand*-1.d0/,whar/nchand*0.d0/,lnar/nchand*-1/
      real *8 cutkin(14)
c 
      real *8 wgt,wgt1
...
*
      real *8 totpt
      integer nx,ninit,i,l,m,lnot,lnot1
      parameter (nx= 20)
      data ninit/0/
      save ninit
      if(ninit.eq.0) then
*
c-       set the number of channels
*
         if (nph.eq.2) then
           nchan= 4
         elseif (nph.eq.1) then
           nchan= 2
         elseif (nph.eq.0) then
           nchan= 1
         else
           print*,'nph=',nph,' not implemented yet'
           stop
         endif
         valu = 1.d0/dfloat(nchan)  
         do l= 1,nchan
           apw(l) = valu
         enddo  
*
...
c
      ...
      bet= 0.d0
      do l= 1,nchan
        if (apw(l).lt.0.d0) then
          print*,'error n. 1 in phspace' 
          stop
        endif 
        lnar(l)= -1
        bet= bet+apw(l) 
 ...
 10   continue       
      if (npk.eq.1) then 
        call momgen(0,nfspart,nph,mw,wwid,mq,roots,x1,x2,pcm,whar(npk),
     +              lnar(npk))
      elseif (npk.eq.2) then
        call momgen1(0,nfspart,nph,mw,wwid,mq,roots,1,x1,x2,pcm,
     +              whar(npk),lnar(npk))
      elseif (npk.eq.3) then
        call momgen1(0,nfspart,nph,mw,wwid,mq,roots,2,x1,x2,pcm,
     +              whar(npk),lnar(npk))
      elseif (npk.eq.4) then
        call momgen2(0,nfspart,nph,mw,wwid,mq,roots,1,x1,x2,pcm,
     +              whar(npk),lnar(npk))
      endif     
      lnot= lnar(npk)
...
*
c 
      factor= 1d0/(2.d0*pi)**(3*(nfspart+nph)+2)/2.d0/s/x1/x2
c 
*
...
*
c 
      do l= 3,nfspart+nph+3
         do m= l+1,nfspart+nph+4
c 
...
*
c 
      call chkcut(lnot,pt,p,eta,dr,nfspart,nph,ihvy)
c 
      if (lnot.eq.1) then
         pswgt= 0.d0
         goto 100
...
*
      ...
      elseif (nph.eq.1) then
        if (npk.eq.1) then 
          call momgen1(1,nfspart,nph,mw,wwid,mq,roots,1,x1,x2,pcm,
     +                 whar(2),lnar(2))
        elseif (npk.eq.2) then
           call momgen(1,nfspart,nph,mw,wwid,mq,roots,x1,x2,pcm,
     +                 whar(1),lnar(1))
        endif
        goto 19 
      endif
*
      if (npk.eq.1) then 
        call momgen1(1,nfspart,nph,mw,wwid,mq,roots,1,x1,x2,pcm,
     +              whar(2),lnar(2))
        call momgen1(1,nfspart,nph,mw,wwid,mq,roots,2,x1,x2,pcm,
     +              whar(3),lnar(3))
        call momgen2(1,nfspart,nph,mw,wwid,mq,roots,1,x1,x2,pcm,
     +              whar(4),lnar(4))
      elseif (npk.eq.2) then
        call momgen(1,nfspart,nph,mw,wwid,mq,roots,x1,x2,pcm,
     +              whar(1),lnar(1))
...
*
 19   dj= 0.d0
      do l= 1,nchan
       if ((lnar(l).eq.0).and.(whar(l).ne.0.d0)) dj= dj+apw(l)/whar(l) 
       if ((lnar(l).lt.0)) then
         print*,'error n. 2 in phspace' 
         stop
       endif 
      enddo 
      wgt= 1.d0/dj 
...
*
      totpt= 0.d0
*
c-    Sum of trasnverse mass mt^2 of jets and photons
*
      do i=3,nfspart+nph+2
      ...
      elseif(iqopt.eq.3) then
         l=  nfspart+nph+3
         qsq=mw**2+(p(1,l)+p(1,l+1))**2+(p(2,l)+p(2,l+1))**2
 ...
 100  continue
      ...
      subroutine chkcut(lnot,pt,p,eta,dr,nfspart,nph,ihvy)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c                                                                c
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 ptphmin,etaphmax,drphjmin,drphmin
      integer n3,n4,i,j,maxpar,ninit,nfspart,nph,lnot,ihvy
      real*8 cutkin(14)
      common/loccut/cutkin
      data ninit/0/
      parameter (maxpar=10)
     ...
     $     ,metmin
      save ptphmin,etaphmax,drphjmin,drphmin
      if(ninit.eq.0) then
         n3= nfspart+1
         n4= nfspart+2
         ninit=1
...
c     require dR(b-jet)<drjmin, dR(jet-jet)<drjmin
       do i=3,nfspart
          if(ihvy.ne.6) then
             if(dr(n3,i).lt.drjmin)        goto 10
             if(dr(n4,i).lt.drjmin)        goto 10
          endif
          do j=i+1,nfspart
...
c
      if(nph.ge.1) then
c     impose minimum pt and require eta within allowed range, for photons
         do i=nfspart+3,nfspart+nph+2
            if (pt(i).lt.ptphmin)           goto 10
            if (abs(eta(i)).gt.etaphmax)    goto 10
         enddo 
         if(ihvy.ne.6) then
c     require dR(b-ph)<drphjmin
            do i=nfspart+3,nfspart+nph+2
               if(dr(n3,i).lt.drphjmin)         goto 10
               if(dr(n4,i).lt.drphjmin)         goto 10
            enddo
         endif
c
c     require dR(ph-ph)<drphmin
         do i= nfspart+3,nfspart+nph+1
            do j= i+1,nfspart+nph+2
               if(dr(i,j).lt.drphmin)      goto 10
            enddo
         enddo 
c
c     require dR(ph-j)<drphjmin
         do i= 3,nfspart
            do j= nfspart+3,nfspart+nph+2
...
*
c 
      subroutine momgen(lflag,nfspart,nph,rm,ga,mq,roots,
     &                  x1,x2,p,wgt,lw)
c 
*
c-    Generator of np particles in the LAB frame.
c-
c-    if lflag.eq.0  the momenta p are generated
c-    if lflag.eq.1  the momenta p are an input to compute wgt
c-
*
      implicit none
      real*8 rm,ga,mq,roots,x1,x2,wgt,zero,rm2,etacut
      real *8 wgt1, wgt2
      real*8 ptjmin,ptbmin,cutkin,etajmax,etabmax,drjmin,s,sw
...
c 
      integer lw,lw1,npar,mpar,np,nfspart,nph,lim,nw,j,k
      real*8 ptphmin,etaphmax
c 
      parameter (npar= 20)
      real*8 pm(0:4,npar),pt0(npar),pt1(npar),xm(npar),xmr(npar),
     .       eta0(npar),pt0_l(npar),pt1_l(npar),p(0:3,*)
      real*8 ranram(111),randec(1:2)
      common/loccut/cutkin(14)
      integer nvar,nv,m,l,nbin,ndummy,mask,mmask,md
      integer nct,nx1,nx2,maxn,lmin,lmax,lflag
      parameter (maxn= 100)
...
*
        ...
        enddo
*
c-      Parameters for the b b_bar jets:
*
        do j= nw-nph-2,nw-nph-1
          pt0_l(j) = ptbmin
          pt1_l(j) = roots/2.d0
          eta0(j)  = etabmax
          xm(j)    = mq        
...
*
      if (np.eq.2) then
        ...
        if (lflag.eq.0) then
          ...
          j= nw+2
          sw= 0.d0 
          sw= sw+(p(0,j)+p(0,j+1))**2
          sw= sw-(p(1,j)+p(1,j+1))**2
          sw= sw-(p(2,j)+p(2,j+1))**2
          sw= sw-(p(3,j)+p(3,j+1))**2
        endif  
        call resonm(lflag,rm,ga,lim,cxmw,cxpw,sw,dj1,ran0)
        ...
        y0 =-0.5d0*log(tau) 
        call ttriangle(lflag,-y0,y0,-y0,y0,yr,wjr,ranram(1),lw1)
        if (lflag.eq.0) then
          x1 = sq*exp(yr) 
          x2 = sq*exp(-yr) 
        endif
...
*
        ...
        call dec2fm(lflag,sw,pm(0,nw),
     .              zero,zero,p(0,nw+2),p(0,nw+3),dj2,randec)
*
        if (lflag.eq.0) then
...
*
     ...
     .            real(nfspart-2)*ptjmin+
     .            2*ptbmin+real(nph)*ptphmin)**2)
*
...
*
      ...
      lw1= 0
      call ppeaka(lflag,tau0,ag,tau0,1.d0,tau,wtau,ranram(2),lw1)
      if (lw1.eq.1) goto 100
      sq = sqrt(tau)
      y0 =-0.5d0*log(tau) 
      lw1= 0
      call ttriangle(lflag,-y0,y0,-y0,y0,yr,wjr,ranram(1),lw1)
...
*
      if (lflag.eq.1) then   
        do j= 1,nw
          do k= 0,3
            if (j.ne.nw) then
              pm(k,j)=  p(k,j+2)/roots
            else
              pm(k,j)= (p(k,j+2)+p(k,j+3))/roots
            endif
          enddo
        enddo
      endif
*
     ...
     +            (wgt1*apw(2)*djbintot+wgt2*apw(1)*djb0*djb1)
        ...
        call mom(1,nw,pt0,pt1,eta0,xmr,en,pz,pm,wgt1,ranram,lw)
        call momr(1,nw,xmr,en,pz,pm,wgt2,lw1)
        if ((lw.eq.1).and.(lw1.eq.1))          goto 100
        if ((wgt1.eq.0.d0).and.(wgt2.eq.0.d0)) goto 100
        djb1= 1.d0
        djb0= 1.d0
        m= 0
        ...
        enddo
        if     ((lw.eq.1) .or.(wgt1.eq.0.d0)) then 
          wgt= wgt2/(apw(2)*djb1)
        elseif ((lw1.eq.1).or.(wgt2.eq.0.d0)) then 
          wgt= wgt1/(apw(1)*djb0*djb1)
        else 
          wgt= wgt1*wgt2/
     +       (wgt1*apw(2)*djb1+wgt2*apw(1)*djb0*djb1)
        endif 
      endif
*
...
*
      subroutine momgen1(lflag,nfspart,nph,rm,ga,mq,roots,nbw,
     +                   x1,x2,pio,wgt,lw)
*
c-    Generator of np particles in the LAB frame.
c- 
c-    the photon nbw is resonating with the Breit-Wigner
c-
c-    if lflag.eq.0  the momenta pio are generated
c-    if lflag.eq.1  the momenta pio are an input to compute wgt
c-
*
      implicit none
      real*8 rm,ga,mq,roots,x1,x2,wgt,zero,rm2,etacut
      ...
      real*8 rootsh,sq,y0,yr,cxmw,cxpw,dj1,dj2,wtau,wjr
      real*8 pt0lmax,pt0lsum,en,pz,eta0min,ran0
      integer lw,lw1,npar,mpar,np,nfspart,nph,lim,nw,j,k,lsub,lflag
      real*8 ptphmin,etaphmax
      parameter (npar= 20)
      real*8 pm(0:4,npar),pt0(npar),pt1(npar),xm(npar),xmr(npar),
     .       eta0(npar),pt0_l(npar),pt1_l(npar),p(0:3,npar)
      real*8 pio(0:3,*)
      real*8 ranram(111)
      real*8 xmrambo(3),prambo(4,3),prt(0:3,3)
      integer kp(1:3,1:3)
      data kp/9*0/
      common/loccut/cutkin(14)
      integer nvar,nv,m,l,nbin,ndummy,mask,mmask
      integer nct,nx1,nx2,maxn,lmin,lmax,nbw
      parameter (maxn= 100)
      real*8 dj,djbintot,djbin,dummy
      real*8 djb0,djb1
      common/psopt/nvar,nv
      real*8 peropt
      common/psopt1/mask(maxn),mmask(maxn),peropt(maxn)
...
*
      if (mpar.eq.0) then
        if (nbw.gt.nph) then
          print*,'error in momgen1: nbpw > npg'
          stop
        endif
        if     (nph.eq.3) then
          kp(1,1)= 2
...
*
        ...
        enddo
*
c-      Parameters for the (W+phot) pseudoparticle:
*
        pt0_l(nw) = zero
        pt1_l(nw) = roots/2.d0
        eta0(nw)  = etacut
        xm(nw)    = zero  ! provisional (W+phot) mass!!!
*
        cxmw      = zero
*
c-      Parameters for the x1,x2 integration:
...
*
        ...
        enddo
*
c-      final state masses for the 3-body isotropic decay
*
        xmrambo(1)= 0.d0 
        xmrambo(2)= 0.d0 
        xmrambo(3)= 0.d0 
*
c-      set n. of variables subtracted from the optimization 
*
        lsub= 2
      endif
*
      if (lflag.eq.1) then
        do j= 1,np
          do l= 0,3
            if ((j.le.nfspart).or.(j.gt.nfspart+nph)) then
              p(l,j+2)= pio(l,j+2)  
            else 
              p(l,nfspart+2+kp(j-nfspart,nbw))= pio(l,j+2)  
            endif
          enddo
        enddo
      endif 
...
*
      if (np.eq.3) then
        ...
        if (lflag.eq.0) then
          ...
          j= nw+2
          sw= 0.d0 
          sw= sw+(p(0,j)+p(0,j+1)+p(0,j+2))**2
          sw= sw-(p(1,j)+p(1,j+1)+p(1,j+2))**2
          sw= sw-(p(2,j)+p(2,j+1)+p(2,j+2))**2
          sw= sw-(p(3,j)+p(3,j+1)+p(3,j+2))**2
        endif  
        call resonm(lflag,rm,ga,lim,cxmw,cxpw,sw,dj1,ran0)
        if (dj1.eq.0.d0) goto 100 
        if (lflag.eq.0) then
...
*
        if (lflag.eq.1) then 
          do j= 1,3 
            call boost(1,sw,pm(0,nw),prt(0,j),p(0,nw+j+1))
            prambo(4,j)=  prt(0,j) 
            prambo(1,j)=  prt(1,j)
            prambo(2,j)=  prt(2,j)
            prambo(3,j)=  prt(3,j)
        ...
        call rambo(lflag,3,sqrt(sw),xmrambo,prambo,dj2)
        if (lflag.eq.0) then 
          do j= 1,3 
            prt(0,j)= prambo(4,j) 
            prt(1,j)= prambo(1,j) 
            prt(2,j)= prambo(2,j) 
            prt(3,j)= prambo(3,j) 
            call boost(0,sw,pm(0,nw),prt(0,j),p(0,nw+j+1))
          enddo
        endif
        if (lflag.eq.1) then
          m= 0
          ...
          do j= 1,np
            do l= 0,3
              if ((j.le.nfspart).or.(j.gt.nfspart+nph)) then
                ...
                pio(l,j+2)= p(l,nfspart+2+kp(j-nfspart,nbw))  
              endif
            enddo
          enddo
        endif 
...
c      call resonm(0,rm,ga,lim,cxmw,cxpw,sw,dj1,ran0)
      call resonm(lflag,rm,ga,lim,cxmw,cxpw,sw,dj1,ran0)
      if (dj1.eq.0.d0) goto 100 
*
c-    The mass of the last particle and the upgrading of xmsum
...
*
      ...
      tau0   = max(tau0,pt0lsum**2/s)
      tau0   = max(tau0,1.d0/s*(
     .            +float(nfspart-2)*ptjmin
     .            +float(2)*ptbmin+float(nph)*ptphmin)**2)
*
c-    Generates x1 and x2: 
*
      if (lflag.eq.1) then
        ...
        do j= 1,nw
          do k= 0,3
            if (j.ne.nw) then
              ...
              pm(k,j)= (p(k,j+2)+p(k,j+3)+p(k,j+4))/roots
            endif
          enddo
        enddo
      endif
*
      call mom(lflag,nw,pt0,pt1,eta0,xmr,en,pz,pm,wgt1,ranram,lw)
      if (lw.eq.1) goto 100
*
      if (lflag.eq.1) then   
        m= 0
...
*
      subroutine momgen2(lflag,nfspart,nph,rm,ga,mq,roots,nbw,
     +                   x1,x2,pio,wgt,lw)
*
c-    Generator of np particles in the LAB frame.
c- 
c-    2 photons are resonating with the Breit-Wigner:
c-    when nph=3, nbw labels the 3 possible permutations 
c-    when nph=2, nbw must be set to 1 (1 only possible permutation)
c-
c-    if lflag.eq.0  the momenta pio are generated
c-    if lflag.eq.1  the momenta pio are an input to compute wgt
c-
*
     ...
     .       eta0(npar),pt0_l(npar),pt1_l(npar),p(0:3,npar)
      real*8 pio(0:3,*)
      real*8 ranram(111)
      real*8 xmrambo(4),prambo(4,4),prt(0:3,4)
      integer kp(1:3,1:3)
      data kp/9*0/
      common/loccut/cutkin(14)
      integer nvar,nv,m,l,nbin,ndummy,mask,mmask
...
*
      if (mpar.eq.0) then
        if (nbw.gt.nph) then
          print*,'error in momgen2: nbpw > npg'
          stop
        endif
        if     (nph.eq.3) then
          kp(1,1)= 1
        ...
        elseif (nph.eq.2) then
          if (nbw.ne.1) then
            print*,'error in momgen2: nph.eq.2 and nbw.ne.1'
            stop 
          stop
          endif 
          kp(1,1)= 1
...
*
        ...
        enddo
*
c-      Parameters for the (W+2phot) pseudoparticle:
*
        pt0_l(nw) = zero
        pt1_l(nw) = roots/2.d0
        eta0(nw)  = etacut
        xm(nw)    = zero  ! provisional (W+2phot) mass!!!
*
        cxmw      = zero
*
c-      Parameters for the x1,x2 integration:
...
*
      if (np.eq.4) then
        ...
        if (lflag.eq.0) then
          ...
          j= nw+2
          sw= 0.d0 
          sw= sw+(p(0,j)+p(0,j+1)+p(0,j+2)+p(0,j+3))**2
          sw= sw-(p(1,j)+p(1,j+1)+p(1,j+2)+p(1,j+3))**2
          sw= sw-(p(2,j)+p(2,j+1)+p(2,j+2)+p(2,j+3))**2
          sw= sw-(p(3,j)+p(3,j+1)+p(3,j+2)+p(3,j+3))**2
        endif  
        call resonm(lflag,rm,ga,lim,cxmw,cxpw,sw,dj1,ran0)
        if (dj1.eq.0.d0) goto 100 
        if (lflag.eq.0) then
...
*
        ...
        call rambo(lflag,4,sqrt(sw),xmrambo,prambo,dj2)
        if (lflag.eq.0) then 
          do j= 1,4 
            prt(0,j)= prambo(4,j) 
            prt(1,j)= prambo(1,j) 
        ...
        do j= 1,nw
          do k= 0,3
            if (j.ne.nw) then
              ...
              pm(k,j)= (p(k,j+2)+p(k,j+3)+p(k,j+4)+p(k,j+5))/roots
            endif
          enddo
        enddo
      endif
...
*
     ...
     .               pm,wgt,ranram,lw)
      ...
      real*8 pt0(*),pt1(*),eta0(*),
     .       xmr(*),bxmr(npar)
      real*8 pm(0:4,*),pt(npar),eta(npar)
...
*
     ...
     .                pm,wgt,lw)
      ...
      parameter (npar= 20)
      real*8 xmr(*),en0,pz0,pm(0:4,*),wgt,dj
...
c
         integer jp
         common/jpr/jp
         integer cls(100)/4*0     ! 2 quarks processes
...
c     + 10,3,2,3,2,  11,3,2,3,2,  10,1,4,1,4,   1,1,4,1,4, !
c     +  2,1,3,1,4,   3,1,4,1,3,
c     + 0,                   ! N of yukawa
c     + 15,                  ! N self-gauge
c     + 1, 2, 3,  10, 2, 3,   4, 2, 3,   5, 1, 1, ! ZWW, AWW, auxiliary
c     + 5, 1,10,   5,10,10,   5, 2, 3,   6, 1, 2, ! auxiliary
...
c     + 825*-100       /     ! EOF signal
         data inpint/ 20,
     >               11,1,1,1,1,  11,1,2,1,2,  11,2,2,2,2,   2,1,1,1,2,  ! guu, gdd, gbb, w+ud
     >                3,1,4,1,3,  11,2,1,2,1,   2,2,1,2,2,                ! w-en, gcc
     >               10,1,1,1,1,  10,1,2,1,2,  10,2,2,2,2,   10,2,1,2,1, ! phot- (u,u) (d,d) (s,s) (c,c)
     >               10,1,4,1,4,   2,1,3,1,4,   3,1,2,1,1,               ! phot-(e,e), W+nubare W-dbaru

     >                3,2,2,2,1,  10,3,2,3,2,  11,3,2,3,2,               ! W-cs, phot-(bb), glu-(bb)
     >                3,3,2,3,1,  10,3,1,3,1,  11,3,1,3,1,               ! W-tb, phot-(tt), glu-(tt)
     >                0,                                                 ! N of yukawa
     >                8,                                                 ! N self-gauge
     >                11,11,11,  12,11,11,  10,2,3,  5,10,10,   
     >                 6,10, 2,   7,10, 3,   4, 2, 3,  5,2,3,                  ! ggg Auxgg
     >                873*-100/
c
c BEGIN_RP
c         if (nqrk.eq.4.or.nqrk.eq.2) then
c         else
c          write(*,*)'nqrk=',nqrk ,'not yet supported'
c         endif
c END_RP
         avgspin=0.d0
c
