...
c
c run parameters:
c write process code, code=6 for b bbar b bbar b bbar + g
      write(niosta,*) 'b bbar b bbar b bbar + g'
      write(niosta,*)
     $     '======================================='
     ...
     $     'Generation cuts for the partonic event sample:' 
      if(njets.gt.0) then
        write(niosta,*) '     Final-state gluon:'
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      implicit none
      double precision ptjmin,drbmin,drjmin,etabmax,etajmax,ptbmin
      integer maxpar,ninit,njets,j,lnot,i,l,iglu
      double precision cutkin(10)
      common/loccut/cutkin
      data ninit/0/
      parameter (maxpar=10)
...
c     impose minimum pt and require eta within allowed range, for b's
      do i=3,8
         ...
         if (abs(eta(i)).gt.etabmax)    goto 10
      enddo 
c     require dR(b-b)>drbmin for all six bottom quarks
      do i=1,5
         icb=i+2
         do j=i+1,6
            jcb=j+2
      ...
      if(njets.gt.0) then
c     impose minimum pt and require eta within allowed range for the gluon
         iglu=njets+6
         if (pt(iglu).lt.ptjmin)           goto 10
         if (abs(eta(iglu)).gt.etajmax)    goto 10
c     require dR(b-g)>drjmin
         do l=3,8
            if(dr(l,iglu).lt.drjmin)       goto 10
         enddo
      endif 
 5    return
