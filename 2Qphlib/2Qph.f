...
c      if(nph.ne.1) then
c         write(*,*) 'nph = ',nph,' not available yet'
c         stop
c      endif
...
c write process code, code=.. for QQbar +jets
      if(njets.gt.0) then
        write(niosta,*) Q(ihvy),Qbar(ihvy),'+',nph,' gamma '
     $       ,'+',njets,' jets'
      else
        write(niosta,*) Q(ihvy),Qbar(ihvy),'+',nph,' gamma '
      endif 
     ...
     $       ,' dR(bb)>',drQmin 
        ...
        write(niosta,*) 'ptmin=',ptphmin,' |etamax|=',etaphmax
     $       ,' dR(gamma-j) dR(gamma-Q) dR(g-g) > ',drphjmin 
      elseif(ihvy.eq.6) then
        write(niosta,*) 'No generation cuts on t tbar'
        write(niosta,*) '     photons:'
        write(niosta,*) 'ptmin=',ptphmin,' |etamax|=',etaphmax
     $       ,' dR(gamma-j) dR(gamma-Q) > ',drphjmin 
      endif
      if(njets.gt.0) then
...
*
      pswgt=0.d0
c      if(ihvy.eq.6) then
c         print*,'not yet available'
c         stop
c         call momgen_ttj(njets,nph,mq,roots,x1,x2,pcm,
c     +                   wgt,lnot)
c      else if(ihvy.le.5) then
         call momgen_bbj(njets,nph,mq,roots,x1,x2,pcm,
     +                   wgt,lnot)
c      endif
...
*
      factor= 1d0/(2.d0*pi)**(3*(njets+nph+2)-4)/2.d0/s/x1/x2
*
...
*
      if(ihvy.eq.6) call chkcut_ttj(lnot,pt,p,eta,dr,njets,nph)
      if(ihvy.le.5) call chkcut_bbj(lnot,pt,p,eta,dr,njets,nph)
      if (lnot.eq.1) then
         pswgt= 0.d0
         goto 100
      endif
...
*
      subroutine chkcut_ttj(lnot,pt,p,eta,dr,njets,nph)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c                                                                c
...
c     require dR(gamma-gamma)>drphmin
      do i=1,nph-1
         do j=i+1,nph
            if(dr(njets+4+i,njets+4+j).lt.drphmin) goto 10
         enddo
      enddo
c     impose minimum pt and require eta within allowed range, for photons
      do i=njets+5,njets+4+nph
...
c     impose minimum pt and require eta within allowed range, for jets
         ...
         enddo 
c     dR(jet-gamma)<drphjmin
         do i=5,njets+4
            do j=1,nph
               if(dr(i,njets+4+j).lt.drphjmin)        goto 10
            enddo
         enddo
c     require dR(jet-jet)<drjmin
 ...
 10   lnot= 1
      ...
      subroutine chkcut_bbj(lnot,pt,p,eta,dr,njets,nph)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c                                                                c
c     Applies kinematical cuts to the final state during the phase
c     -space generation                                          c
...
c     impose minimum pt and require eta within allowed range, for photons
      do i=njets+5,njets+4+nph
         ...
         if (abs(eta(i)).gt.etaphmax)    goto 10
      enddo 
c     require dR(b-gamma)<drphjmin
      do i=1,nph
         if(dr(3,njets+4+i).lt.drphjmin)        goto 10
         if(dr(4,njets+4+i).lt.drphjmin)        goto 10
      enddo
c
      if(njets.gt.0) then
c     impose minimum pt and require eta within allowed range, for jets
         do i=5,njets+4
            ...
            if (abs(eta(i)).gt.etajmax)    goto 10
         enddo 
c     dR(b-jet)<drjmin, dR(jet-jet)<drjmin, dR(jet-gamma)<drphjmin
         do i=5,njets+4
...
*
      subroutine momgen_bbj(njets,nph,qm,roots,x1,x2,p,wgt,lw)
*
...
*
      ...
      real*8 ptphmin,etaphmax,drphjmin
      ...
      real*8 xmsum,ptlim
      integer lw,npar,mpar,np,njets,nph,lflag,lim,j,l,k,lw1
      parameter (npar= 20)
...
*
        ...
        rcnmin= min(2.d0*ptjmin*sin(drjmin/2.d0),
     +              2.d0*ptphmin*sin(drphjmin/2.d0))
*
c-      l is the number of light jets + gamma
*
        l= njets+nph
...
*
     ...
     +          (2d0*qm+float(njets)*ptjmin+float(nph)*ptphmin)**2)
*
...
*
      subroutine momgen_ttj(njets,nph,qm,roots,x1,x2,p,wgt,lw)
*
c-    Generator of np particles in the LAB frame.
*
      implicit none
      ...
      integer lw,lw1,npar,mpar,np,njets,nph,nw,j,k
      parameter (npar= 20)
...
*
      ...
      if (md.eq.1) djbintot= djbintot*djb0 
*
c-    njets= 0 and nph= 0: problems
*
      if ((njets+nph).eq.0) then
        write(*,*) 'njets+nph=0'
        stop
      endif
*
...
c      if(ihvy.le.5) then
         nv  = 2*(njets+nph+2)-1
c      elseif(ihvy.eq.6) then
c         nv  = 2*(njets+nph+2)->1
c      else
c         print*,'wrong value of ihvy in setgrid'
c         stop
c      endif
      nvar= nv*ngrid
      do n= 2,nvar+1
...
C
         data inpint/12,
***     >               2,1,1,1,2,  ! w+ud
     >           10,3,2,3,2,    10,2,1,2,1,  10,2,2,2,2,  10,1,1,1,1,  !Abb, Acc, Ass, Auu
     >           10,1,2,1,2,    10,3,1,3,1,                            !Add, Att
