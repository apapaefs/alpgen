...
c
c run parameters:
c write process code, code=14 for W+jets+photons
      if(njets.eq.0) then
...
c     $  ,' m_(ph-ph)>',mggmin
        write(niosta,*) 'dR(ph-j)>',drphjmin,' dR(ph-l)>',drphlmin  
...
c 
      do j=1,nph
         ptphot(j)=pt(njets+j+2)
         etaphot(j)=eta(njets+j+2)
         do i=j+1,nph
            drphotphot(i,j)=dr(njets+i+2,njets+j+2)
            drphotphot(j,i)=drphotphot(i,j)
         enddo
      enddo
      do j=1,njets
        do i=1,nph
           drjphot(j,i)=dr(j+2,i+2+njets)
        enddo
      enddo
c 
c leptons
...
c        read(*,*) idecmode
        if (idecmode.gt.4) then
          write(*,*) 'iwdecmode=',idecmode,
...
c
 ...
 10   continue       
      if (npk.eq.1) then 
        call momgen(0,njets,nph,mw,wwid,roots,x1,x2,pcm,whar(npk),
     +              lnar(npk))
      elseif (npk.eq.2) then
        call momgen1(0,njets,nph,mw,wwid,roots,1,x1,x2,pcm,whar(npk),
     +              lnar(npk))
      elseif (npk.eq.3) then
        call momgen1(0,njets,nph,mw,wwid,roots,2,x1,x2,pcm,whar(npk),
     +              lnar(npk))
      elseif (npk.eq.4) then
        call momgen2(0,njets,nph,mw,wwid,roots,1,x1,x2,pcm,whar(npk),
     +              lnar(npk))
      endif     
      lnot= lnar(npk)
...
*
c 
      factor= 1d0/(2.d0*pi)**(3*(njets+nph)+2)/2.d0/s/x1/x2
c 
*
...
*
      ...
      elseif (nph.eq.1) then
        if (npk.eq.1) then 
          call momgen1(1,njets,nph,mw,wwid,roots,1,x1,x2,pcm,whar(2),
     +                 lnar(2))
        elseif (npk.eq.2) then
           call momgen(1,njets,nph,mw,wwid,roots,x1,x2,pcm,whar(1),
     +                 lnar(1))
        endif
        goto 19 
      endif
*
      if (npk.eq.1) then 
        call momgen1(1,njets,nph,mw,wwid,roots,1,x1,x2,pcm,whar(2),
     +              lnar(2))
        call momgen1(1,njets,nph,mw,wwid,roots,2,x1,x2,pcm,whar(3),
     +              lnar(3))
        call momgen2(1,njets,nph,mw,wwid,roots,1,x1,x2,pcm,whar(4),
     +              lnar(4))
      elseif (npk.eq.2) then
        call momgen(1,njets,nph,mw,wwid,roots,x1,x2,pcm,whar(1),
     +              lnar(1))
...
*
      ...
      elseif(iqopt.eq.3) then
         qsq=mw**2+(p(1,njets+nph+3)+p(1,njets+nph+4))**2
     +            +(p(2,njets+nph+3)+p(2,njets+nph+4))**2
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 ptlmin,metmin,mpp
c 
      real*8 ptphmin,etaphmax,drphjmin,drphmin
      integer maxpar,ninit,njets,j,lnot,i,nph
c 
c 
      real*8 cutkin(14)
c 
...
c
c      if(nph.gt.1) then
c         mpp=     (p(4,njets+3)+p(4,njets+4))**2      
c         mpp= mpp-(p(1,njets+3)+p(1,njets+4))**2      
c         mpp= mpp-(p(2,njets+3)+p(2,njets+4))**2      
c         mpp= mpp-(p(3,njets+3)+p(3,njets+4))**2      
c         mpp= sqrt(dabs(mpp))
c         if (mpp.lt.mggmin) goto 10
c      endif
c
...
c 
      subroutine momgen(lflag,njets,nph,rm,ga,roots,x1,x2,p,wgt,lw)
c 
*
...
c 
      integer lw,lw1,npar,mpar,np,njets,nph,lim,nw,j,k
...
*
     ...
     .            +float(njets)*ptjmin+float(nph)*ptphmin)**2)
*
...
*
      subroutine momgen1(lflag,njets,nph,rm,ga,roots,nbw,
     +                   x1,x2,pio,wgt,lw)
*
c-    Generator of np particles in the LAB frame.
c- 
...
*
      ...
      real*8 rootsh,sq,y0,yr,cxmw,cxpw,dj1,dj2,wtau,wjr
      real*8 pt0lmax,pt0lsum,en,pz,eta0min,ran0
      integer lw,lw1,npar,mpar,np,njets,nph,lim,nw,j,k,lsub,lflag
      real*8 ptphmin,etaphmax
      parameter (npar= 20)
      real*8 pm(0:4,npar),pt0(npar),pt1(npar),xm(npar),xmr(npar),
...
*
      if (lflag.eq.1) then
        do j= 1,np
          do l= 0,3
            if ((j.le.njets).or.(j.gt.njets+nph)) then
              p(l,j+2)= pio(l,j+2)  
            else 
              p(l,njets+2+kp(j-njets,nbw))= pio(l,j+2)  
            endif
          enddo
        enddo
      endif 
          ...
          do j= 1,np
            do l= 0,3
              if ((j.le.njets).or.(j.gt.njets+nph)) then
                ...
                pio(l,j+2)= p(l,njets+2+kp(j-njets,nbw))  
              endif
            enddo
          enddo
        endif 
...
*
      subroutine momgen2(lflag,njets,nph,rm,ga,roots,nbw,
     +                   x1,x2,pio,wgt,lw)
*
c-    Generator of np particles in the LAB frame.
c- 
...
c
     ...
     >               10,1,4,1,4,   2,1,3,1,4,   3,1,2,1,1,    
     >                3,2,2,2,1,
     >                0,                                                 ! N of yukawa
     >                7,                                                 ! N self-gauge
     >                11,11,11,  12,11,11,  10,2,3,  5,10,10,   
     >                 6,10, 2,   7,10, 3,   4,2,3,                     ! ggg Auxgg
     >                901*-100/
c
