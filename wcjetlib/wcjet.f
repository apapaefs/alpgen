...
c       jgrid= 1   -->   q qbar and qbar q 
c       jgrid= 2   -->   g q    and g qbar
c       jgrid= 3   -->   q g    and qbar g
c       jgrid= 4   -->   g g
c       jgrid= 5   -->   qq->qq
      data jgrid/2,3,4,1,5,3*1,5,3,2,2*3,2*2,2*4,83*0/
      integer i
c
...
c     process input parameters
      ...
      nprtns= nfspart
      nw=1
      if(nfspart.eq.1) then
         jprocmax=2
         ngrid   = 3
      elseif(nfspart.eq.2) then
         jprocmax= 9
         ngrid   = 5
      elseif(nfspart.eq.3) then
         jprocmax=15
         ngrid   = 5
      elseif(nfspart.ge.4) then
         jprocmax=17
         ngrid   = 5
      endif 
...
c
c run parameters:
c write process code for W+c+jets
      if(nfspart.eq.0) then
        write(niosta,*) 'somethig wrong with njets'
        stop
      else
        write(niosta,*) 'W + c + ',nfspart-1,' jets'
      endif
      write(niosta,*) 'W-> ell nu'
      write(niosta,*) '======================================='
      write(niosta,*)
     $     'Generation cuts for the partonic event sample:' 
      write(niosta,*) '     Charm jets:'
      write(niosta,*) 'ptmin=',ptcmin,' |etamax|=',etacmax
     $       ,' dR(c-j)>',drjmin 
      write(niosta,*) '     Light jets:'
      if(nfspart.gt.1) then
...
c     jproc
c     1  g q -> q' W    and    g qbar -> q'bar W
c     2  q g -> q' W    and  qbar g -> q'bar W
c     3  gg -> q qbar' W              
c     4  q(bar) q(bar)'' -> W q'(bar) q(bar)''  
c     5  q''(bar) q(bar) -> W q'(bar) q''(bar)
c     6  q'' q''bar -> W q q'bar
c     7  q qbar -> W q qbar'  + c.c.
c     8  q qbar -> W q' qbar
c     9  q q -> W q q'
c     10 q g -> W q' q'' qbar'' ,   qbar g -> W qbar' q'' qbar''
c     11 g q -> W q' q'' qbar'' ,   g qbar -> W qbar' q'' qbar''
c     12 q g -> W q q qbar'
c     13 q g -> W q' q qbar
c     14 g q -> W q q qbar' 
c     15 g q -> W q' q qbar
c     16 gg -> q qbar' q'' qbar'' W              
c     17 gg -> q qbar q qbar' W  + c.c.
c-----------------------------------------------------------------
      implicit none
      include 'alpgen.inc'
      include 'wcjet.inc'
...
c     
c     jproc=1   njets>=0
c     g q -> q' W    and    g qbar -> qbar' W   (q'(bar) = c)
      if(jproc.eq.1) then
        do i=-3,3, 2
          do j=1,2
            if(abs(icab(i,j)).eq.4) then
              icount=icount+1
...
c     
c     jproc=2   njets>=0
c     q g -> q' W    and  qbar g -> qbar' W   (q', qbar' = c)
...
c     d g -> u W-
        ...
        enddo 
c     jproc=3   njets>=1
c     gg -> q qbar' W    (q or qbar' = c)             
      elseif(jproc.eq.3) then
        do i=1,4
          do j=1,2
            if(abs(i).eq.4.or.abs(icab(i,j)).eq.4) then
              icount=icount+1
              tmp(icount)=fcab(j)
              slum=slum+tmp(icount)
            endif
...
c     
c     jproc=4 1002  njets>=1
c     q(bar) q(bar)'' -> W q'(bar) q(bar)''  (q' = c(bar))
          ...
          if(i.ne.0) then
            do j=1,2
              ...
              do k=1,4
                ...
                if((abs(is*iqpp(ik,j,abs(i)))).ne.4.and.
     >               abs(icab(i,j)).eq.4) then
                  icount=icount+1
                  tmp(icount)=f1(i)*f2(is*iqpp(ik,j,abs(i)))
     >                 *fcab(j)
                  slum=slum+tmp(icount)
                endif
              enddo
...
c     q''(bar) q(bar) -> W q'(bar) q''(bar) (q' = c(bar))
     ...
     >               abs(icab(i,j)).eq.4) then
                  icount=icount+1
                  tmp(icount)=f1(is*iqpp(ik,j,abs(i)))
     >                 *f2(i)*fcab(j)
                  slum=slum+tmp(icount)
                endif
              enddo
...
c     
c     jproc=6 njets>=1
c     q'' q''bar -> W q q'bar  (q or q'bar = c(bar))
              ...
              if(k.ne.abs(i)) then
                do j=1,2
                  if(icab(k,j).ne.abs(i)) then
                    if(abs(k).eq.4.or.abs(icab(k,j)).eq.4) then
                      icount=icount+1
...
c     
c     jproc=7 njets>=1
c     q qbar -> W q qbar'  + c.c. (qbar' = c(bar))
c     
...
c     
c     jproc=16  njets>=3
c     gg -> q'' qbar'' q qbar' W   (q or q' = c(bar),  q'' . ne. c/cbar)
      elseif(jproc.eq.16) then
        do i=1,4
          do j=1,2
            do k=1,2
              if((abs(i).eq.4.or.abs(icab(i,j)).eq.4).and.
     >             abs(iqpp(k,j,abs(i))).ne.4) then
                icount=icount+1
                tmp(icount)=fcab(j)
                slum=slum+tmp(icount)
              endif
...
c     
c     jproc=17  njets>=3
c     gg -> q qbar q qbar' W  + c.c. (qbar' = cbar)
      elseif(jproc.eq.17) then
c     do i=1,4    ! MLM Aug 16 2006
        do i=-3,3
          if(i.ne.0) then
            do j=1,2
              if(abs(icab(i,j)).eq.4) then
...
c     identical quarks in final state
      if(jproc.eq.12.or.jproc.eq.14.or.jproc.eq.17) xlum=0.5d0*xlum 
c     effco=ccoef(#quark pairs,#gluons)
      if(jproc.le.3) then
        nlqp=1
      elseif(jproc.le.17) then
        nlqp=2
      else
...
c jets first
      ...
      do j=1,nfspart
         ptj(j)=pt(j+2)
         etaj(j)=eta(j+2)
         if(abs(ifl(j+2)).eq.4) then
           icha=j
           if(ptj(j).lt.ptcmin) goto 10
           if(abs(etaj(j)).gt.etacmax) goto 10
         else
           if(ptj(j).lt.ptjmin) goto 10
           if(abs(etaj(j)).gt.etajmax) goto 10
         endif
         do i=j+1,nfspart
            drjj(i,j)=dr(i+2,j+2)
            drjj(j,i)=drjj(i,j)
...
c     boosts variables
      double precision gamma,beta,e0,em,pz,deltae,eresc
...
c
      do ip=1,npart
        ...
        do ic=1,2
          icuwrt(ic,ip)=icu(ic,ip)
        enddo
        if(abs(ifl(ip)).eq.4.and.ip.gt.2) then
c assign charm mass to final state charms
          pwrt(5,ip)=dmass(4)
c rescale initial state energy to enforce P(in)=P(out)
          pz=p(3,ip)
          e0=p(4,ip)
          beta=(x1-x2)/(x1+x2)
          gamma=0.5d0*(x1+x2)/sqrt(x1*x2)
          em=gamma*(e0-beta*pz)
          deltae=sqrt(em**2+dmass(4)**2)-em
          eresc=1d0+deltae/sqrt(s*x1*x2)
          x1=x1*eresc
          x2=x2*eresc
          do i=1,2
            pwrt(3,i)=eresc*pwrt(3,i)
            pwrt(4,i)=eresc*pwrt(4,i)
          enddo
          do il=1,2
            pwrt(il,ip)=p(il,ip)
          enddo
          pwrt(3,ip)=p(3,ip) + gamma*beta*deltae
          pwrt(4,ip)=p(4,ip) + gamma*deltae
        else
          do il=1,5
            pwrt(il,ip)=p(il,ip)
          enddo
...
c          tmp=tmp-pwrt(ip,i)
c        enddo
c        if(abs(tmp).gt.1d-3) write(*,*) 'deltaP(',ip,')=',tmp
c      enddo
      end
...
c                                                                c
c  h(1)  h(2) -> h(3) ...  h(nfspart) e nu                         c
c                                                                c
...
*
      pswgt=0.d0
*
      call momgen(nfspart,mw,wwid,roots,x1,x2,pcm,wgt,lnot)
...
*
      call chkcut(lnot,pt,p,eta,dr,nfspart)
      if (lnot.eq.1) then
         pswgt= 0.d0
         goto 100
      endif
...
*
      ...
      elseif(iqopt.eq.1) then
        qsq=mw**2+(p(1,nfspart+3)+p(1,nfspart+4))**2+(p(2,nfspart+3)+p(2
     $        ,nfspart+4))**2
 ...
 100  continue
      ...
      subroutine chkcut(lnot,pt,p,eta,dr,nfspart)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c                                                                c
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 ptcmin,etacmax,ptmin,etamax
      ...
      save ninit,ptjmin,ptcmin,etajmax,etacmax,drjmin,ptlmin
     $     ,metmin,ptmin,etamax
      if(ninit.eq.0) then
         ninit=1
         ptjmin=cutkin(1)
         ...
         ptlmin=cutkin(7)
         metmin=cutkin(10)
         ptmin=min(ptjmin,ptcmin)
         etamax=max(etajmax,etacmax)
      endif
      lnot= 0

c     impose minimum pt and require eta within allowed range, for jets;
c     will apply the separate charm/light-jet cuts after the parton
C     representing the charm has been selected in selflav
      do i=3,nfspart+2
         if (pt(i).lt.ptmin)           goto 10
         if (abs(eta(i)).gt.etamax)    goto 10
      enddo 
...
c     
 10   lnot= 1
      ...
      subroutine momgen(nfspart,rm,ga,roots,x1,x2,p,wgt,lw)
*
...
*
      ...
      real*8 pt0lmax,pt0lsum,en,pz,eta0min,ran0,wgt1,wgt2
...
*
      if (mpar.eq.0) then
        ...
        s      = roots*roots
*
        ptjmin = min(cutkin(1),cutkin(2))
        etajmax= max(cutkin(3),cutkin(4))
        drjmin = cutkin(5)
*
